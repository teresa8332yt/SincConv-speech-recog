...
class SpeechCommandDataset(Dataset):
    def __init__(self, data_path='/Desktop/CodeFolder/KWS/google dataset/speech_commands_v0.01/', is_training=True, transform=None):
        ...
        if is_training:
            ...
            self.data_list_path = os.path.join(self.data_path, 'valid_list.txt')

        self.ids = [id.strip() for id in open(self.data_list_path)]
        self.classes = classes =['zero','one','two','three','four','five','six','seven','eight','nine','on','off','up','down',
                                 'visual','tree','bed','stop','learn','yes','no','forward','backward','dog','bird','happy',
                                 'marvin','left','right','go','cat','follow','house','sheila','wow']
